...
/***/ function(module, exports) {
	...
	;
	(function (w) {
		//w.baseUrl = '//192.168.3.227:8080/core';
		w.baseUrl = '//192.168.3.246:8080/core';
		w.MenuMap = [{
			title: '首页',
			url: '/home'
		}, {
...
/***/ function(module, exports) {
	...
	})(window, function init($, undefined) {
	  ...
	  return (el.offsetWidth > 0 || el.offsetHeight > 0);*/
		...
		_.call(function () {
			...
			_.msie = function () {
				...
				if (isNaN(msie)) return _.undefined;else return msie;
			};
		});
		...
		_.unparam = function (input) {
			...
			if ((temp = $.type(input)) != 'string' || temp == 'string' && !temp.length) return {};
			...
			if (!(temp = items.length) || temp == 1 && temp === '') return result;
			...
			$.each(items, function (index, item) {
				...
				while (temp = expBrackets.exec(key)) subitems.push(temp[1]);
				...
				if (!temp || !(key = temp[1]) || !key.length) return;
				...
				$.each(subitems, function (subindex, subitem) {
					...
					if (subindex == size) {
						...
						// Si la anidación no existe

						// Se crea un objeto con su respectivo enlace

						link = link[temp] = {};
					} else {
						// Si la anidación existe
		...
		_.caseIf = function (_case) {
			...
			if (!ret && len % 2 === 0) return arguments[len - 1];
			if (typeof ret === 'function') return ret();else return ret;
		};
		_.escape = function () {
			var htmlEscapes = {
				'&': '&amp;',
				...
				_.each(obj, function (v, k) {
					...
					switch (_t) {
						case "object":
							...
							break;
						case "string":
							if (!path.isReserv(k)) obj[k] = _.escape(v);
					}
				});
			}
			return _escape(obj);
			...
			return {
				on: function (eventName, handlers, data, one) {
					...
					for (idx = 0, length = eventNames.length; idx < length; idx++) {
						...
						if (handler) {
							if (one) handler = replaceHandler(orgEventName, handler);
				...
				trigger: function (eventName, e) {
					...
					var that = this,
					    ...
					    callbacks,
					    length;
					if (events.length === 0) return that;
					e = e || {};
					e.sender = that;
					...
					while (true) {
						...
						if (callbacks && callbacks.length) {
							for (var i = 0, len = callbacks.length; i < len; i++) {
								if (callbacks[i].data) callbacks[i].handler.call(that, callbacks.data, e);else callbacks[i].handler.call(that, e);
							}
						}
					}
					return that;
	...
	(function (_) {
		...
		Clazz.extend = function (proto, cnstrctr) {
			var Base = function () {},
				...
				if (cnstrctr) this[cnstrctr].apply(this, arguments);else that.apply(this, arguments);
			},
		...
		function wrapfunc(wraper) {
			function _rap(obj) {
				if (!obj) return undefined;else if (obj.option) return $.proxy(obj.func, obj.context, obj.option);else return $.proxy(obj.func, obj.context);
			}
	...
	if (!String.prototype.format) {
		String.prototype.format = function (arg1, arg2) {
			...
			function readV(obj, prop) {
				var tmpObj = obj[prop],
				    len;
				if (tmpObj || tmpObj === '') return tmpObj;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var routes = {
	    ...
	    '/integrated': __webpack_require__(26),
	    '/category': __webpack_require__(28),
	    '/reminder': __webpack_require__(25),
	    '/complaint': __webpack_require__(22)
	};
...
/***/ function(module, exports) {
	...
	module.exports = "<% _.each(data.list,function(item){ %>\r\n\t<a href=\"#<%- item.url%>\"><%- item.title%></a>\r\n<%})%>";
